const items = [{
    "id": 1,
    "name": "AZITHROMYCIN",
    "link": "AZITHROMYCIN",
    "mainCategory": "Suspensions",
    "subCategory": "Antibiotic"
},
{
    "id": 2,
    "name": "OFLOXACIN",
    "link": "OFLOXACIN",
    "mainCategory": "Suspensions",
    "subCategory": "Antibiotic"
},
{
    "id": 3,
    "name": "OFLOXACIN + TINIDAZOLE",
    "link": "OFLOXACIN0",
    "mainCategory": "Suspensions",
    "subCategory": "Antibiotic"
},
{
    "id": 4,
    "name": "OFLOXACIN + ORNIDAZOLE",
    "link": "OFLOXACIN1",
    "mainCategory": "Suspensions",
    "subCategory": "Antibiotic"
},
{
    "id": 5,
    "name": "ROXITHROMYCIN",
    "link": "ROXITHROMYCIN",
    "mainCategory": "Suspensions",
    "subCategory": "Antibiotic"
},
{
    "id": 6,
    "name": "OFLOXACIN +  NITOZOXANIDE",
    "link": "OFLOXACIN2",
    "mainCategory": "Suspensions",
    "subCategory": "Antibiotic"
},
{
    "id": 7,
    "name": "OFLOXACIN +  METRONIDAZOLE",
    "link": "OFLOXACIN3",
    "mainCategory": "Suspensions",
    "subCategory": "Antibiotic"
},
{
    "id": 8,
    "name": "PCM    +   CPM    +   PHENYLEPHRINE HCL     +   SODIUM CITRATE     +   MENTHOL",
    "link": "PCM4",
    "mainCategory": "Suspensions",
    "subCategory": "Anticold"
},
{
    "id": 9,
    "name": "CPM    +   PCM   +   PHENYLEPHERINE HCL",
    "link": "CPM5",
    "mainCategory": "Suspensions",
    "subCategory": "Anticold"
},
{
    "id": 10,
    "name": "CETIRIZINE HCL    +   PCM     +   PHENYLEPHERINE HCL",
    "link": "CETIRIZINE6",
    "mainCategory": "Suspensions",
    "subCategory": "Anticold"
},
{
    "id": 11,
    "name": "CETIRIZINE DIHCL",
    "link": "CETIRIZINE7",
    "mainCategory": "Suspensions",
    "subCategory": "Anticold"
},
{
    "id": 12,
    "name": "LEVOCETIRIZINE DIHCL",
    "link": "LEVOCETIRIZINE8",
    "mainCategory": "Suspensions",
    "subCategory": "Anticold"
},
{
    "id": 13,
    "name": "DICYCLOMINE HCL + PCM",
    "link": "DICYCLOMINE9",
    "mainCategory": "Suspensions",
    "subCategory": "Anticold"
},
{
    "id": 14,
    "name": "FEXOFENADINE HCL",
    "link": "FEXOFENADINE10",
    "mainCategory": "Suspensions",
    "subCategory": "Anticold"
},
{
    "id": 15,
    "name": "DEXTROMETHORPHAN HYDROBROMIDE + CHLORPHENIRAMINE MALEATE",
    "link": "DEXTROMETHORPHAN11",
    "mainCategory": "Suspensions",
    "subCategory": "Anticold"
},
{
    "id": 16,
    "name": "DEFLAZACORT",
    "link": "DEFLAZACORT",
    "mainCategory": "Suspensions",
    "subCategory": "Analgesic & Anti-Inflammatory"
},
{
    "id": 17,
    "name": "PROMETHAZINE HCL",
    "link": "PROMETHAZINE12",
    "mainCategory": "Suspensions",
    "subCategory": "Analgesic & Anti-Inflammatory"
},
{
    "id": 18,
    "name": "PCM     +   PROMETHAZINE HCL",
    "link": "PCM13",
    "mainCategory": "Suspensions",
    "subCategory": "Analgesic & Anti-Inflammatory"
},
{
    "id": 19,
    "name": "MEGALDRATE   +   SIMETHICON",
    "link": "MEGALDRATE14",
    "mainCategory": "Suspensions",
    "subCategory": "Antacid"
},
{
    "id": 20,
    "name": "OXETACAINE    +   ALUMINIUM HYDRIXIDE   +   MAGNESIUM HYDROXIDE",
    "link": "OXETACAINE15",
    "mainCategory": "Suspensions",
    "subCategory": "Antacid"
},
{
    "id": 21,
    "name": "ACTIVATED DIMETHICON    +   MAGNESIUM HYDRO    +   DRIED ALIMINIUM HYDRO",
    "link": "ACTIVATED16",
    "mainCategory": "Suspensions",
    "subCategory": "Antacid"
},
{
    "id": 22,
    "name": "AMOXYCILLIN",
    "link": "AMOXYCILLIN",
    "mainCategory": "Syrups",
    "subCategory": "Dry Syrup"
},
{
    "id": 23,
    "name": "CEFIXIME",
    "link": "CEFIXIME",
    "mainCategory": "Syrups",
    "subCategory": "Dry Syrup"
},
{
    "id": 24,
    "name": "CEFPODOXIME",
    "link": "CEFPODOXIME",
    "mainCategory": "Syrups",
    "subCategory": "Dry Syrup"
},
{
    "id": 25,
    "name": "CEFIXIME + OFLOXACIN",
    "link": "CEFIXIME0",
    "mainCategory": "Syrups",
    "subCategory": "Dry Syrup"
},
{
    "id": 26,
    "name": "IRON(III)HYDROXIDE POLYMALTOSE COMPLEX  +  FOLIC ACID",
    "link": "IRON(III)HYDROXIDE1",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 27,
    "name": "FERRIC AMMONIUM CITRATE    +  CYANOCOBALAMIN    +  FOLIC ACID    +  CUPRIC SULPHATE   +  MANGANESE SULPHATE",
    "link": "FERRIC2",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 28,
    "name": "MULTIFORMULA \u2026\u2026\u2026. (FOOD)",
    "link": "MULTIFORMULA3",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 29,
    "name": "FERROUS ASCORBATE   +  FOLIC ACID",
    "link": "FERROUS4",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 30,
    "name": "FERRIC AMMONIUM CITRATE    +  FOLIC ACID    +   CYANOCOBALAMINE   +  SORBITOL SOL (70%)  Q.S.",
    "link": "FERRIC5",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 31,
    "name": "IRON (III) HYDROXIDE POLYMALTOSE",
    "link": "IRON6",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 32,
    "name": "ENZYME FORMULA (IN FOOD)",
    "link": "ENZYME7",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 33,
    "name": "CALCIUM  +  D3   +   B12",
    "link": "CALCIUM8",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 34,
    "name": "CALCIUM CARBONATE  (EQ. TO ELEMENTAL CALCIUM)   +    D3",
    "link": "CALCIUM9",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 35,
    "name": "FERRIC AMMONIUM CITRATE  (EQ TO ELEMENTAL IRON )  +   FOLIC ACID    +  CYANOCOBALAMIN",
    "link": "FERRIC10",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 36,
    "name": "FERRIC AMM. CIT.  (EQ. TO ELEMENTAL IRON )  +  CYANOCO.    +  FOLIC ACID   + ZINC SUL.  +  PYRIDOXINE HCL   +  SORBITOL (70%)",
    "link": "FERRIC11",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 37,
    "name": "PYRIDOXINE HCL   +  NICOTINAMIDE   +  CYANOCOBALAMIN   +   LYSINE HCL",
    "link": "PYRIDOXINE12",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 38,
    "name": "LYCOPENE FORMULA \u2026\u2026\u2026\u2026\u2026\u2026.(IN FOOD)",
    "link": "LYCOPENE13",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 39,
    "name": "ALPHA AMYLASE   ( BACTERIAL 1:800)  +   PAPAIN",
    "link": "ALPHA14",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 40,
    "name": "L-LYSINE   + NICOTINAMIDE     +  D-PANTHENOL   +   RIBO. SOD. PHOS.   +  THIAMINE HCL   + PYRIDOXINE HCL    +  CYANOC. (IN FOOD)",
    "link": "L-LYSINE15",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 41,
    "name": "CYPROHEPTADINE HCL    +   TRICHOLINE CITRATE",
    "link": "CYPROHEPTADINE16",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 42,
    "name": "TRICHOLINE CITRATE    SORBITOL SOLUTION (70%)  Q.S",
    "link": "TRICHOLINE17",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 43,
    "name": "FERROUS GLUCONATE   (EQ. TO  ELEMENTAL IRON)  +   FOLIC ACID   +  B12     +   CALCIUM LACTATE",
    "link": "FERROUS18",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 44,
    "name": "PROTEIN FORMULA\u2026\u2026\u2026\u2026\u2026\u2026\u2026(IN DRUG)",
    "link": "PROTEIN19",
    "mainCategory": "Syrups",
    "subCategory": "Tonic Syrup"
},
{
    "id": 45,
    "name": "DOMPERIDONE",
    "link": "DOMPERIDONE",
    "mainCategory": "Syrups",
    "subCategory": "Drops"
},
{
    "id": 46,
    "name": "ONDANSETRON",
    "link": "ONDANSETRON",
    "mainCategory": "Syrups",
    "subCategory": "Drops"
},
{
    "id": 47,
    "name": "DIPHENHYDRAMINE + AMONIUM CHLORIDE + SODIUM CITRATE + MENTHOL",
    "link": "DIPHENHYDRAMINE20",
    "mainCategory": "Syrups",
    "subCategory": "Cough Syrup"
},
{
    "id": 48,
    "name": "TERBUTALINE SULPHATE  +  GUAIPHENESIN  +   BROMHEXINE HCL      +  MENTHOL",
    "link": "TERBUTALINE21",
    "mainCategory": "Syrups",
    "subCategory": "Cough Syrup"
},
{
    "id": 49,
    "name": "TERBUTALINE SULPHATE      +  GUAIPHENESIN    +  AMBROXOL HCL     +  MENTHOL",
    "link": "TERBUTALINE22",
    "mainCategory": "Syrups",
    "subCategory": "Cough Syrup"
},
{
    "id": 50,
    "name": "DIPHENHYDRAMINE HCL  +  TERPINE HYDRATE  +   AMMONIUM CHLORIDE  +  SODIUM CITRATE +   MENTHOL",
    "link": "DIPHENHYDRAMINE23",
    "mainCategory": "Syrups",
    "subCategory": "Cough Syrup"
},
{
    "id": 51,
    "name": "DEXTROMETHORPHAN HBR  +  PHENYLEPHRINE HCL  +  CPM",
    "link": "DEXTROMETHORPHAN24",
    "mainCategory": "Syrups",
    "subCategory": "Cough Syrup"
},
{
    "id": 52,
    "name": "DEXTROMETHORPHAN HBR    +  PHENYLEPHRINE HCL    +  CPM    +  PCM",
    "link": "DEXTROMETHORPHAN25",
    "mainCategory": "Syrups",
    "subCategory": "Cough Syrup"
},
{
    "id": 53,
    "name": "DEXTROMETHORPHAN HBR  +   CPM",
    "link": "DEXTROMETHORPHAN26",
    "mainCategory": "Syrups",
    "subCategory": "Cough Syrup"
},
{
    "id": 54,
    "name": "CETIRIZINE DIHCL      +   AMBROXOL HCL",
    "link": "CETIRIZINE27",
    "mainCategory": "Syrups",
    "subCategory": "Cough Syrup"
},
{
    "id": 55,
    "name": "AMBROXOL HCL   +  GUAIPHENESIN   +  PHENYLEPHERINE HCL   +  CPM  +  MENTHOL",
    "link": "AMBROXOL28",
    "mainCategory": "Syrups",
    "subCategory": "Cough Syrup"
},
{
    "id": 56,
    "name": "CPM    +  SODIUM CITRATE  +  AMMONIUM CHLORIDE",
    "link": "CPM29",
    "mainCategory": "Syrups",
    "subCategory": "Cough Syrup"
},
{
    "id": 57,
    "name": "DEXTROMETHORPHEN HBR    +  CPM     +   GUAIPHENESIN     +  AMMONIUM CHLORIDE",
    "link": "DEXTROMETHORPHEN30",
    "mainCategory": "Syrups",
    "subCategory": "Cough Syrup"
},
{
    "id": 58,
    "name": "DEXTROMETHORPHAN HBR   +  CETIRIZINE HCL     +  PHENYLEPHRINE HCL      +  MENTHOL",
    "link": "DEXTROMETHORPHAN31",
    "mainCategory": "Syrups",
    "subCategory": "Cough Syrup"
},
{
    "id": 59,
    "name": "AMBROXOL HCL   +  LEVOCETIRIZINE HCL   +  GUAIPHENESIN    +  PHENYLEPHRINE HCL",
    "link": "AMBROXOL32",
    "mainCategory": "Syrups",
    "subCategory": "Cough Syrup"
},
{
    "id": 60,
    "name": "DMR    +  AMMONIUM  +  BROMHEXINE   +  MENTHOL",
    "link": "DMR33",
    "mainCategory": "Syrups",
    "subCategory": "Cough Syrup"
},
{
    "id": 61,
    "name": "SODIUM CHLORIDE + BENZOLEKONIUM + PHENYLMERCURIC ACETATE",
    "link": "SODIUM0",
    "mainCategory": "External Preparations",
    "subCategory": "Nasal Sprays"
},
{
    "id": 62,
    "name": "XYLOMETAZOLINE HCL NASAL DROPS & SPRAY",
    "link": "XYLOMETAZOLINE1",
    "mainCategory": "External Preparations",
    "subCategory": "Nasal Sprays"
},
{
    "id": 63,
    "name": "POTASSIUM  NITRATE    +  SODIUM FLUORIDE",
    "link": "POTASSIUM2",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 64,
    "name": "POTASSIUM  NITRATE",
    "link": "POTASSIUM3",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 65,
    "name": "DICLOFENAC DIETHYLAMINE  EQ. DICOFENAC SODIUM",
    "link": "DICLOFENAC4",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 66,
    "name": "ADAPALENE    +  CLINDAMYCIN PHOSPHATE",
    "link": "ADAPALENE5",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 67,
    "name": "CLINDAMYCIN PHOSPHATE  +  NICOTINAMIDE",
    "link": "CLINDAMYCIN6",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 68,
    "name": "MOMETASONE FUROATE",
    "link": "MOMETASONE7",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 69,
    "name": "DICLOFENAC DIETHYLAMINE   +  METHYL SALICYLATE    +  MENTHOL   +  LINSEED OIL    +  BENZYL ALCOHOL",
    "link": "DICLOFENAC8",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 70,
    "name": "CLOBETASOL PROPIONATE    +  NEOMYCIN SULPHATE     +  MICONAZOLE NITRATE    +  CHLOROCRESOL",
    "link": "CLOBETASOL9",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 71,
    "name": "FLUTICASONE  PROPIONATE    +   IMIDUREA",
    "link": "FLUTICASONE10",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 72,
    "name": "FLUTICASONE  PROPIONATE",
    "link": "FLUTICASONE11",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 73,
    "name": "UREA    +  LACTIC ACID   +  PROPYLENE GLYCOL    +  LIQUID PARAFFIN   +  METHYL PARABEN  +  PROPYPARABEN",
    "link": "UREA12",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 74,
    "name": "BECLOMETHASONE DIPROPIONATE   +  SALICYLIC ACID",
    "link": "BECLOMETHASONE13",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 75,
    "name": "BECLOMETHASONE DIPROPIONATE    +  CLOTRIMAZOLE    +  NEOMYCIN SULPHATE    +  CHLOROCRESOL",
    "link": "BECLOMETHASONE14",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 76,
    "name": "CLOTRIMAZOLE   +  BECLOMETHASONE DIPROPIONATE",
    "link": "CLOTRIMAZOLE15",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 77,
    "name": "OFLOXACIN    +  ORNIDAZOLE   +  TERBINAFINE HCL   +  CLOBETASOL PROPIONATE   +  METHYLPARABEN   +  PROPYLPARABEN",
    "link": "OFLOXACIN16",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 78,
    "name": "OFLOXACIN +  FLUOCINOLONE ACETONIDE   +  CLOTRIMAZOLE    +  CHLOROCRESOL",
    "link": "OFLOXACIN17",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 79,
    "name": "ACRIFLAVINE    +  THYMOL   +  CETRIMIDE",
    "link": "ACRIFLAVINE18",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 80,
    "name": "CLOBETASOL PROPIONATE    +  SALICYLIC ACID",
    "link": "CLOBETASOL19",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 81,
    "name": "BECLOMETHASONE DIPROPIONATE   +  KETOCONAZOLE",
    "link": "BECLOMETHASONE20",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 82,
    "name": "PAPAIN    +  UREA",
    "link": "PAPAIN21",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 83,
    "name": "SODIUM HYALURONATE",
    "link": "SODIUM22",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 84,
    "name": "HYDROQUINONE+  TERTINOIN +  MOMETASONE FUROATE",
    "link": "HYDROQUINONE23",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 85,
    "name": "HYDROQUINONE+  TERTINOIN + FLUOCINOLONE",
    "link": "HYDROQUINONE24",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 86,
    "name": "LIGNOCAINE HCL GEL",
    "link": "LIGNOCAINE25",
    "mainCategory": "External Preparations",
    "subCategory": "Ointments"
},
{
    "id": 87,
    "name": "KETOCONAZOLE",
    "link": "KETOCONAZOLE",
    "mainCategory": "External Preparations",
    "subCategory": "Shampoo"
},
{
    "id": 88,
    "name": "PERMETHRIN LOTION",
    "link": "PERMETHRIN26",
    "mainCategory": "External Preparations",
    "subCategory": "Lotions"
},
{
    "id": 89,
    "name": "GAMABENZENE LOTION",
    "link": "GAMABENZENE27",
    "mainCategory": "External Preparations",
    "subCategory": "Lotions"
},
{
    "id": 90,
    "name": "DICLOFENAC DIETHYLAMINE   +  METHYL SALICYLATE    +  MENTHOL   +  LINSEED OIL    +  BENZYL ALCOHOL",
    "link": "DICLOFENAC28",
    "mainCategory": "External Preparations",
    "subCategory": "Roll On"
},
{
    "id": 91,
    "name": "DISODIUM HYDROGEN CITRATE",
    "link": "DISODIUM0",
    "mainCategory": "Miscellaneous Products",
    "subCategory": ""
},
{
    "id": 92,
    "name": "CHLORHEXIDINE GLUCONATE",
    "link": "CHLORHEXIDINE1",
    "mainCategory": "Miscellaneous Products",
    "subCategory": ""
},
{
    "id": 93,
    "name": "GLYCERIN",
    "link": "GLYCERIN",
    "mainCategory": "Miscellaneous Products",
    "subCategory": ""
},
{
    "id": 94,
    "name": "SUCRALFATE  +  OXETACAINE",
    "link": "SUCRALFATE2",
    "mainCategory": "Miscellaneous Products",
    "subCategory": ""
},
{
    "id": 95,
    "name": "MILK OF MAGNESIA    +  LIQUID PARAFFIN",
    "link": "MILK3",
    "mainCategory": "Miscellaneous Products",
    "subCategory": ""
},
{
    "id": 96,
    "name": "SUCRALFATE",
    "link": "SUCRALFATE",
    "mainCategory": "Miscellaneous Products",
    "subCategory": ""
},
{
    "id": 97,
    "name": "SODIUM PICOSULFATE",
    "link": "SODIUM4",
    "mainCategory": "Miscellaneous Products",
    "subCategory": ""
},
{
    "id": 98,
    "name": "DICLOFENAC DIETHYLAMINE   +  METHYL SALICYLATE    +   MENTHOL +  LINSEED OIL  +  BENZYL ALCOHOL",
    "link": "DICLOFENAC5",
    "mainCategory": "Miscellaneous Products",
    "subCategory": ""
},
{
    "id": 99,
    "name": "CHOLECALCIFEROL(VITAMIN D3) SHOTS",
    "link": "CHOLECALCIFEROL(VITAMIN6",
    "mainCategory": "Miscellaneous Products",
    "subCategory": ""
},
{
    "id": 100,
    "name": "ETORICOXIB  + THIOCOLCHICOSIDE",
    "link": "http://dmpharma.co.in/Etoricoxibthiocolchicoside.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 101,
    "name": "ARTEMETHER + LUMEFANTRINE",
    "link": "http://dmpharma.co.in/Arthemeter%20+%20Lumefentrine.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 102,
    "name": "DICLOFENAC SODIUM  + THIOCOLCHICOSIDE",
    "link": "http://dmpharma.co.in/diclothio.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 103,
    "name": "CEFIXIME",
    "link": "http://dmpharma.co.in/Cefixime.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 104,
    "name": "CEFIXIME  + OFLOXACIN",
    "link": "http://dmpharma.co.in/Cefixime%20+Ofloxacin.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 105,
    "name": "CEFIXIME + DICLOXACILLIN(ER)",
    "link": "http://dmpharma.co.in/Cefixime%20+%20Dicloxacillin(ER).html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 106,
    "name": "CEFPODOXIME + DICLOXACILLIN(ER)",
    "link": "http://dmpharma.co.in/Cefpodoxime%20+%20Dicloxacillin.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 107,
    "name": "CEFPODOXIME + OFLOXACIN",
    "link": "http://dmpharma.co.in/Cefpodoxime%20+%20Ofloxacin.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 108,
    "name": "CEFPODOXIME",
    "link": "http://dmpharma.co.in/Cefpodoxime.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 109,
    "name": "LORNOXICAM FILM COATED",
    "link": "http://dmpharma.co.in/Lornoxicam.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 110,
    "name": "LORNOXICAM(SR)",
    "link": "http://dmpharma.co.in/Lornoxicam(SR).html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 111,
    "name": "LORNOXICAM + PARACETAMOL",
    "link": "http://dmpharma.co.in/Lornoxicam%20+Paracetamol.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 112,
    "name": "THIOCOLCHICOSIDE TAB/CAP",
    "link": "http://dmpharma.co.in/Thiocholchicoside.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 113,
    "name": "ACECLOFENAC + THIOCOLCHICOSIDE",
    "link": "http://dmpharma.co.in/Aceclofenac+Thiocholchicoside.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 114,
    "name": "ETODOLAC",
    "link": "http://dmpharma.co.in/etodolac.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 115,
    "name": "OLMESARTAN",
    "link": "http://dmpharma.co.in/olmesartan.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 116,
    "name": "DEFLAZACORT",
    "link": "http://dmpharma.co.in/deflazacort.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 117,
    "name": "ROSUVASTATIN",
    "link": "http://dmpharma.co.in/Rosuvastatin.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 118,
    "name": "FEXOFENADINE + MONTELUKAST",
    "link": "http://dmpharma.co.in/fexotab.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 119,
    "name": "LEVOCETIRIZINE + MONTELUKAST",
    "link": "http://dmpharma.co.in/levotab.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 120,
    "name": "URSODEOXYCHOLIC ACID",
    "link": "http://dmpharma.co.in/ursodeoxycholic.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 121,
    "name": "OMEPRAZOLE + ONDANSETRON",
    "link": "http://dmpharma.co.in/ome.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 122,
    "name": "CEFUROXIME AXTEIL",
    "link": "http://dmpharma.co.in/cefuroxime.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 123,
    "name": "VOGLIBOSE",
    "link": "http://dmpharma.co.in/Voglibose.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 124,
    "name": "VOGLIBOSE  + METFORMIN",
    "link": "http://dmpharma.co.in/Voglibose%20+%20Metformin.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 125,
    "name": "METHYLCOBALAMIN + GABAPENTIN",
    "link": "http://dmpharma.co.in/Methylcobalamingabapentin.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 126,
    "name": "TRANXEMIC ACID + MEFENAMIC ACID",
    "link": "http://dmpharma.co.in/tranxemicmefnamic.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 127,
    "name": "LEVOSULPIRIDE",
    "link": "http://dmpharma.co.in/Levosulpride.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 128,
    "name": "LANSOPRAZOLE",
    "link": "http://dmpharma.co.in/Lansoprazole.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 129,
    "name": "TELMISARTAN + HYDROCHLOROTHIAZIDE",
    "link": "http://dmpharma.co.in/TelmisartanHydrochlorothiazide.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 130,
    "name": "SILYMARINE WITH ANTIOXIDANTS",
    "link": "http://dmpharma.co.in/Silymarin.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 131,
    "name": "BIOTIN + NETTLE LEAF  + SAW PALMETTO \n+ PABA",
    "link": "http://dmpharma.co.in/Nettleleaf.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 132,
    "name": "GINSENG + IRON + CALCIUM + VITAMINS + MINERALS",
    "link": "http://dmpharma.co.in/revitalwomen.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 133,
    "name": "GLUCOSAMINE + CHONDROITIN",
    "link": "http://dmpharma.co.in/glucosaminechondroitin.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 134,
    "name": "OLMESARTAN MEDOXOMIL + AMLODIPINE",
    "link": "http://dmpharma.co.in/olmesartanamlodipine.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 135,
    "name": "METHYLPREDNISOLONE",
    "link": "http://dmpharma.co.in/methylprednisolone.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 136,
    "name": "CITICOLINE",
    "link": "http://dmpharma.co.in/Citicoline.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 137,
    "name": "L-GLUTATHIONE + ALPHA LIPOIC ACID",
    "link": "http://dmpharma.co.in/t3.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 138,
    "name": "L-5-METHYLTETRAHYDROFOLATE CALCIUM",
    "link": "http://dmpharma.co.in/t4.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 139,
    "name": "TERBINAFINE",
    "link": "http://dmpharma.co.in/t7.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 140,
    "name": "RACECADOTRIL",
    "link": "http://dmpharma.co.in/t6.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 141,
    "name": "L-GLUTATHIONE+ ALPHA LIPOIC ACID + GRAPE SEED EXTRACT + VITAMIN C + VITAMIN E",
    "link": "http://dmpharma.co.in/L-glutathione,alpha-lipoic acid, vitamin C,vitamin E, grape seed extract.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 142,
    "name": "CRANBERRY + D-MANNOSE",
    "link": "http://dmpharma.co.in/Cranberry%20and%20D-Mannose%20tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 143,
    "name": "L-CARNITINE L - TARTARATE + MECOBALAMIN + FOLIC ACID",
    "link": "http://dmpharma.co.in/L-Carnitine%20and%20Combination%20tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 144,
    "name": "GLUCOSAMINE + CHONDROITIN + COLLAGEN +  GINGER EXTRACT",
    "link": "http://dmpharma.co.in/Glucosamine-Chondroitin-Collagen-Ginger%20Extract%20Tablets.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 145,
    "name": "VITAMINS + MINERALS + AMINO ACIDS + COLLAGEN",
    "link": "http://dmpharma.co.in/t18.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 146,
    "name": "FERROUS ASCORBATE + FOLIC ACID + ZINC SULPHATE",
    "link": "http://dmpharma.co.in/ferrousfolic.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 147,
    "name": "S-ADENOSYL L-METHIONINE",
    "link": "http://dmpharma.co.in/t19.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 148,
    "name": "IPRIFLAVONE + ALFACALCIDOL + CALCIUM CARBONATE",
    "link": "http://dmpharma.co.in/Ipriflavone,Alfacalcidol%20with%20Calcium%20Carbonate%20Tablets.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 149,
    "name": "L-METHYL FOLATE + L-ARGININE + L-CARNITINE + PYRIDOXAL-5 + OMEGA-3 FATTY ACIDS + MECOBALAMIN + BENFOTIAMINE + N- ACETYL CYSTINE + ZINC + VITAMIN D3",
    "link": "http://dmpharma.co.in/L-Methyl%20Folate%20and%20its%20Combination%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 150,
    "name": "LYCOPENE + DHA + MULTIVITAMIN +  MULTIMINERAL",
    "link": "http://dmpharma.co.in/Lycopene,Antioxidants,DHA%20&%20Multivitamin%20with%20Multimineral%20Tablets.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 151,
    "name": "CEFIXIME + AZITHROMYCIN",
    "link": "http://dmpharma.co.in/Cefixime%20+%20Azithromycin.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 152,
    "name": "ATORVASTATIN + EZETIMIBE",
    "link": "http://dmpharma.co.in/AtorvastatinEzetimibe.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 153,
    "name": "SECNIDAZOLE + FLUCONAZOLE + AZITHROMYCIN",
    "link": "http://dmpharma.co.in/t17.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 154,
    "name": "CEFIXIME + ORNIDAZOLE",
    "link": "http://dmpharma.co.in/Cefixime%20+%20Ornidazole.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 155,
    "name": "ALBENDAZOLE + IVERMECTIN",
    "link": "http://dmpharma.co.in/t5.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 156,
    "name": "CHOLECALCIFEROL",
    "link": "http://dmpharma.co.in/Cholecalciferol%20tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 157,
    "name": "GAMMA-LINOLENIC + AMINO ACID + GRAPE SEED + GREEN TEA + COMBINATION",
    "link": "http://dmpharma.co.in/Grape%20Seed%20,%20Green%20Tea%20,%20Gamma%20Linolenic%20Tablets.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 158,
    "name": "FOLIC ACID + DHA + LYCOPENE + ZINC",
    "link": "http://dmpharma.co.in/folicaciddha.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 159,
    "name": "COLLAGEN PEPTIDE + SODIUM HYALURONATE + CHONDROITIN + VITAMIN C",
    "link": "http://dmpharma.co.in/t10.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 160,
    "name": "VITAMINS + MINERALS + NATURAL EXTRACTS + BIOTIN + AMINO ACID",
    "link": "http://dmpharma.co.in/Vitamins%20+%20Minerals%20+%20Natural%20Extracts%20+%20Biotin%20&%20Amino%20Acid%20Tablet%20Advanced%20Formula%20For%20Healthy%20Hair.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 161,
    "name": "CRANBERRY + VITAMIN C + VITAMIN B5  + L-METHIONINE",
    "link": "http://dmpharma.co.in/Cranberry%20%20and%20Vitamin%20C%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 162,
    "name": "MIFEPRISTONE + MISOPROSTOL",
    "link": "http://dmpharma.co.in/MTP%20%28Medical%20Termination%20of%20Pregnancy%29.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 163,
    "name": "MYO-INOSITOL + L-METHYL FOLATE CALCIUM + VITAMIN D3",
    "link": "http://dmpharma.co.in/Myo-inositol,L-methyl%20Folate%20Calcium%20and%20Vitamin%20D3%20Tablets.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 164,
    "name": "MYO-INOSITOL + D-CHIRO-INOSITOL + VITAMIN D3 + CHROMIUM",
    "link": "http://dmpharma.co.in/Myo-inositol%20+%20D-chiro-Inositol%20+%20Vitamin%20D3%20+%20Chromium%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 165,
    "name": "THYROXINE SODIUM",
    "link": "http://dmpharma.co.in/THYROXINE%20SODIUM%20TABLET.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 166,
    "name": "DUTASTERIDE",
    "link": "http://dmpharma.co.in/Dutasteride%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 167,
    "name": "DICLOFENAC + METAXALONE",
    "link": "http://dmpharma.co.in/Diclofenac%20+%20Metaxalone%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 168,
    "name": "ISOXSUPRINE",
    "link": "http://dmpharma.co.in/Isoxsuprine%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 169,
    "name": "NORFLOXACIN  +   TINIDAZOLE",
    "link": "http://dmpharma.co.in/norfloxacin%20and%20tinidazole%20tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 170,
    "name": "METHYLCOBALAMIN +  L-METHYLFOLATE  + PYRIDOXAL",
    "link": "http://dmpharma.co.in/Methylcobalamin,%20l-methylfolate%20and%20pyridoxal-5-phosphate%20tablets.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 171,
    "name": "LEVONORGESTREL",
    "link": "http://dmpharma.co.in/Levonorgestrel.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 172,
    "name": "GREEN TEA + COMBINATION + VITAMINS + MINERALS + AMINO ACIDS",
    "link": "http://dmpharma.co.in/Green%20Tea%20+%20Combination%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 173,
    "name": "TAMSULOSIN HYDROCHLORIDE",
    "link": "http://dmpharma.co.in/Tamsulosin%20Hydrochloride%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 174,
    "name": "TAMSULOSIN + DUTOSTERIDE",
    "link": "http://dmpharma.co.in/Tamsulosin%20and%20Dutasteride%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 175,
    "name": "ARTESUNATE +  SULFADOXINE + PYRIMETHAMINE",
    "link": "http://dmpharma.co.in/Artesunate, Sulfadoxine & Pyrimethamine Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 176,
    "name": "COLLAGEN + NATURAL EXTRACTS + AMINO ACIDS + VITAMINS + MINERALS",
    "link": "http://dmpharma.co.in/Collagen,%20Natural%20Extracts,%20Amino%20Acids,%20Vitamins%20&%20Minerals%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 177,
    "name": "VORICONAZOLE",
    "link": "http://dmpharma.co.in/Voriconazole%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 178,
    "name": "ETODOLAC + PARACETAMOL",
    "link": "http://dmpharma.co.in/Etodolac%20and%20Paracetamol%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 179,
    "name": "CALCIUM CITRATE +  MAGNESIUM + ZINC + VITAMIN D3",
    "link": "http://dmpharma.co.in/Calcium%20Citrate,Magnesium,zinc%20and%20Vitamin%20D3%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 180,
    "name": "SAME +  METHYLCOBALAMIN + L-5 METHYLTETRAHYDROFOLATE CALCIUM",
    "link": "http://dmpharma.co.in/S-Adenosyl%20L-Methionine%20%28SAMe%29%20+%20Methylcobalamin%20+%20L-5%20Methyltetrahydrofolate%20Calcium.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 181,
    "name": "BLEND OF EXTRACT OF SCUTELLARIA BAICALENSIS + ACACIA CATECHU",
    "link": "http://dmpharma.co.in/Blend%20of%20Extract%20of%20Scutellaria%20Baicalensis%20&%20Acacia%20Catechu%20Tablets.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 182,
    "name": "NATURAL EXTRACTS + OMEGA 3 FATTY ACIDS + COMBINATION",
    "link": "http://dmpharma.co.in/Natural%20Extracts,%20Omega%203%20Fatty%20Acids,%20Lactic%20Acid%20Bacillus,%20Vitamin,%20Mineral%20and%20Trace%20Element%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 183,
    "name": "ACEBROPHYLLINE SUSTAINED RELEASE",
    "link": "http://dmpharma.co.in/Acebrophylline%20Sustained%20Release%20Tablets.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 184,
    "name": "METHYLCOBALAMIN +  L-METHYLFOLATE + PYRIDOXAL-5-PHOSPHATE + VITAMIN D3",
    "link": "http://dmpharma.co.in/Methylcobalamin,%20L-Methylfolate,%20Pyridoxal-5-phosphate,%20Vitamin%20D3%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 185,
    "name": "ALPHA LIPOIC ACID + GREEN TEA EXTRACT + COMBINATION",
    "link": "http://dmpharma.co.in/Green tea Extract, Isoflavones, Vitamin C, Coenzyme Q10, Pine Bark Extract, Lycopene, Alpha Lipoic Acid and Mecobalamin Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 186,
    "name": "VITAMIN K 2-7 +  CALCIUM CARBONATE + CALCITRIOL + BORON + ZINC",
    "link": "http://dmpharma.co.in/vitamin.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 187,
    "name": "VITAMIN K2-7 +  METHYLCOBALAMIN + CHOLECALCIFEROL + CALCIUM CITRATE",
    "link": "http://dmpharma.co.in/Vitamin%20K2-7,%20Methylcobalamin,%20Cholecalciferol%20&%20Calcium%20Citrate%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 188,
    "name": "VITAMIN K2-7 +  METHYLCOBALAMIN + CHOLECALCIFEROL + CALCITRIOL",
    "link": "http://dmpharma.co.in/Vitamin%20K2%207,Cholecalciferol+Combination%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 189,
    "name": "D-RIBOSE +  L-CARNITINE + UBIQUINOL + MAGNESIUM",
    "link": "http://dmpharma.co.in/D-Ribose,%20L-Carnitine,%20Ubiquinol%20and%20Magnesium%20Tablets.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 190,
    "name": "L-METHYL FOLATE + COMBINATION",
    "link": "http://dmpharma.co.in/L-Methyl%20Folate%20and%20its%20Combination%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 191,
    "name": "DUTASTERIDE",
    "link": "http://dmpharma.co.in/Dutasteride%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 192,
    "name": "LORNOXICAM +  THIOCOLCHICOSIDE",
    "link": "http://dmpharma.co.in/Lornoxicam%20+Thiocolchicoside.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 193,
    "name": "FENOFIBRATE",
    "link": "http://dmpharma.co.in/Fenofibrate%20Tablets.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 194,
    "name": "BERBERINE +  PURSLANE + GYMNEMA SYLVESTRE + BANABA LEAVES EXTRACT  + MULTIVITAMINS + MINERALS",
    "link": "http://dmpharma.co.in/Berberine,%20Purslane,%20Gymnema%20sylvestre%20and%20Banaba%20leaves%20extract,%20Multivitamins%20and%20Minerals%20Tablet.html\n",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 195,
    "name": "CALCIUM ASPARTATE",
    "link": "http://dmpharma.co.in/Calcium%20Aspartate%20Tablets.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 196,
    "name": "DULOXETINE HYDROCHLORIDE",
    "link": "http://dmpharma.co.in/Duloxetine%20Hydrochloride%20Tablets.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 197,
    "name": "CALCITRIOL + VITAMIN K2-7 + L-METHYLFOLATE + MECOBALAMIN + COMBINATION",
    "link": "http://dmpharma.co.in/Calcitriol,Vitamin%20K2-7,L-Methylfolate,Mecobalamin%20&%20Combination%20Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 198,
    "name": "MILK CALCIUM + MAGNESIUM + VITAMIN C + VITAMIN D3 + MENAQUINONE",
    "link": "http://dmpharma.co.in/Milk Calcium, Magnesium, Vitamin C, Vitamin D3 and Menaquinone Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 199,
    "name": "CINNARIZINE",
    "link": "http://dmpharma.co.in/Cinnarizine%20tablets.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 200,
    "name": "SILYMARINE",
    "link": "http://dmpharma.co.in/Silymarin%20Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 201,
    "name": "PARACETAMOL",
    "link": "http://dmpharma.co.in/PARADIN-650.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 202,
    "name": "L GLUTATHIONE + VITAMIN C",
    "link": "http://dmpharma.co.in/L%E2%80%93GlutathioneandVitamin%20C%20Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 203,
    "name": "MISOPROSTOL",
    "link": "http://dmpharma.co.in/Misoprostol%20Tablet_200%20mcg.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 204,
    "name": "FERROUS ASPARTO GLYCINATE + L-METHYLFOLATE CALCIUM + METHYLCOBALAMIN + PYRIDOXAL-5-PHOSPHATE + CHOLECALCIFEROL",
    "link": "http://dmpharma.co.in/Ferrous%20Asparto%20Glycinate,%20L-Methylfolate%20Calcium,%20Methylcobalamin,%20Pyridoxal-5-Phosphate%20and%20Cholecalciferol%20Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 205,
    "name": "GRAPE SEED EXTRACT + VITAMINS + MINERALS",
    "link": "http://dmpharma.co.in/Grape%20Seed%20Extracts,%20Vitamins,%20Minerals%20Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 206,
    "name": "CALCIUM CITRATE + VITAMIN D3",
    "link": "http://dmpharma.co.in/Calcium%20Citrate%20&%20Vitamin%20D3%20Tablets.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 207,
    "name": "METHYLCOBALAMIN + ALPHA LIPOIC ACID + MYO-INOSITOL + BENFOTIAMINE +  PYRIDOXAL-5-PHOSPHATE + L-METHYLFOLATE + CHROMIUM PICOLINATE",
    "link": "http://dmpharma.co.in/Methylcobalamin_alpha%20lipoic%20acid_Myo-Inositol_Benfotiamine,%20pyridoxal-5-phosphate_l-methylfolateandchromium%20picolinate%20Tablets.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 208,
    "name": "GINSENG EXTRACT POWDER + GINKGO BILOBA POWDER + GARCINIA CAMBOGIA + CITRUS BIOFLAVONOIDS + GREEN TEA EXTRACT + GINGER GARLIC POWDER + LYCOPENE +  COMBINATION",
    "link": "http://dmpharma.co.in/Ginseng%20Extract%20Powder_Ginkgo%20biloba%20powder_Garcinia%20Cambogia_Citrus%20bioflavonoids_Green%20tea%20extract_Ginger_Garlic%20powder_Lycopene_Vegetarian_Formula_Tablet_and_Combination_Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 209,
    "name": "ROSUVASTATIN + FENOFIBRATE",
    "link": "http://dmpharma.co.in/Rosuvastatin_and_Fenofibrate_%20Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 210,
    "name": "CALCIUM ASPARTO GLYCINATE + VITAMIN D3",
    "link": "http://dmpharma.co.in/Calcium%20Asparto%20Glycinate%20%20&%20Vitamin%20D3%20Tablets.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 211,
    "name": "SUPEROXIDE DISMUTASE",
    "link": "http://dmpharma.co.in/Superoxide%20Dismutase%20Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 212,
    "name": "MAGNESIUM BISGLYCINATE + L-CARNITINE L-TARTRATE",
    "link": "http://dmpharma.co.in/Magnesium%20Bisglycinate%20and%20L-carnitine%20L-tartrate%20Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 213,
    "name": "REPAGLINIDE",
    "link": "http://dmpharma.co.in/Repaglinide_0.5mg_Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 214,
    "name": "GINKGO BILOBA + VITAMIN E",
    "link": "http://dmpharma.co.in/Ginkgo%20Biloba_%20Vitamin%20E%20_Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 215,
    "name": "FERROUS ASPARTO GLYCINATE + L-METHYLFOLATE CALCIUM + METHYLCOBALAMIN",
    "link": "http://dmpharma.co.in/Ferrous%20Asparto%20Glycinate,%20L-Methylfolate%20Calcium,%20Methylcobalamin%20Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 216,
    "name": "CO-ENZYME Q10 + LYCOPENE POWDER + AMINO ACID",
    "link": "http://dmpharma.co.in/Co-Enzyme%20Q10,%20Lycopene%20Powder%20and%20Amino%20Acid%20Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 217,
    "name": "FERROUS BISGLYCINATE + MANGANESE + VITAMIN B2 + VITAMIN B12 + COPPER + FOLIC ACID",
    "link": "http://dmpharma.co.in/Ferrous%20Bisglycinate,%20Manganese,%20Vitamin%20B2,%20Vitamin%20B12%20with%20Copper%20and%20Folic%20Acid%20Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 218,
    "name": "FERROUS GLUCONATE + ASTAXANTHIN, ZINC + L \u2013 ARGININE + PYRIDOXINE HCL + LYCOPENE + FOLIC ACID + SELENIUM + VITAMIN B12",
    "link": "http://dmpharma.co.in/Ferrous%20Gluconate,%20Astaxanthin,%20Zinc,%20L%20%E2%80%93%20Arginine,%20Pyridoxine%20HCl,%20Lycopene,%20Folic%20Acid,%20Selenium%20and%20Vitamin%20B12%20Tablets.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 219,
    "name": "FERROUS ASCORBATE + L \u2013 LYSINE + LYCOPENE + FOLIC ACID + METHYLCOBALAMIN + VITAMIN D3 + ZINC + SELENIUM",
    "link": "http://dmpharma.co.in/Ferrous%20Ascorbate,%20L%20%E2%80%93%20Lysine,%20Lycopene,%20Folic%20Acid,%20Methylcobalamin,%20Vitamin%20D3,%20Zinc%20and%20Selenium%20Tablets.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 220,
    "name": "FEXOFENADINE",
    "link": "http://dmpharma.co.in/Fexofenadine%20Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 221,
    "name": "PROANTHOCYANIDIN + LUTEIN + ZEAXANTHIN + VITAMINS + MINERALS",
    "link": "http://dmpharma.co.in/Ferrous%20Gluconate,%20Astaxanthin,%20Zinc,%20L%20%E2%80%93%20Arginine,%20Pyridoxine%20HCl,%20Lycopene,%20Folic%20Acid,%20Selenium%20and%20Vitamin%20B12%20Tablets.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 222,
    "name": "L-CYSTINE + L-METHIONINE + PABA + BIOTIN + VITAMINS + ZINC",
    "link": "http://dmpharma.co.in/L-Cystine,%20L-Methionine,%20PABA,%20Biotin,%20Vitamins%20and%20Zinc%20Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 223,
    "name": "ROSE HIP EXTRACT + GLUCOSAMINE SULPHATE SODIUM + CHONDROITIN SULPHATE + COLLAGEN PEPTIDE TYPE II + VITAMIN C + SODIUM HYALURONATE",
    "link": "http://dmpharma.co.in/Rose%20Hip%20Extract,%20Glucosamine%20Sulphate%20Sodium,%20Chondroitin%20Sulphate,%20Collagen%20Peptide%20Type%20II,%20Vitamin%20C%20and%20Sodium%20Hyaluronate%20Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 224,
    "name": "L-CARNITINE L-TARTARATE + CO ENZYME Q10 + ZINC + PIPERINE",
    "link": "http://dmpharma.co.in/L-Carnitine%20L-Tartarate,%20%20Co%20Enzyme%20Q10,%20%20Zinc,%20Piperine%20tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 225,
    "name": "METHYLCOBALAMIN + VITAMIN B1 + ALPHALIPOIC ACID + FOLIC ACID + VITAMIN D3",
    "link": "http://dmpharma.co.in/Methylcobalamin,%20Vitamin%20B1,%20Alphalipoic%20Acid,%20Folic%20Acid%20and%20Vitamin%20D3%20Tablet.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 226,
    "name": "BURDOCK EXTRACT + ASTAXANTHIN + BETACAROTENE + GRAPE SEED EXTRACT + VITAMINS + MINERALS + L\u2013 CYSTINE",
    "link": "http://dmpharma.co.in/Burdock%20Extract,%20Astaxanthin,%20Betacarotene,%20Grape%20Seed%20Extract,%20Vitamins,%20Minerals%20and%20L%20%E2%80%93%20Cystine%20Tablets.html",
    "mainCategory": "Tablets",
    "subCategory": ""
},
{
    "id": 227,
    "name": "ACECLOFENAC + RABEPRAZOLE",
    "link": "http://dmpharma.co.in/c2.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 228,
    "name": "LEVOSULPIRIDE  + RABEPRAZOLE",
    "link": "http://dmpharma.co.in/Levosulpride.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 229,
    "name": "RABEPRAZOLE  + DOMPERIDONE",
    "link": "http://dmpharma.co.in/RabeprazoleDomperidone.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 230,
    "name": "PRE-PRO BIOTIC",
    "link": "http://dmpharma.co.in/Pre-Pro%20Biotic.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 231,
    "name": "ORLISTAT",
    "link": "http://dmpharma.co.in/orlistat.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 232,
    "name": "DICLOFENAC SODIUM  + THIOCOLCHICOSIDE",
    "link": "http://dmpharma.co.in/DiclofenacThicolchicoside.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 233,
    "name": "MECOBALAMIN,BENFOTIAMINE AND ITS COMBINATION",
    "link": "http://dmpharma.co.in/meco1000.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 234,
    "name": "AMINO ACIDS + VITAMINS",
    "link": "http://dmpharma.co.in/aminoacidandvitamins.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 235,
    "name": "FERROUS FUMARATE + FOLIC ACID  +\n B12 + ZINC",
    "link": "",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 236,
    "name": "LYCOPENE WITH MULTIVITAMIN CAPSULE",
    "link": "http://dmpharma.co.in/Lycopene.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 237,
    "name": "GINSENG WITH MULTIVITAMIN CAPSULE",
    "link": "http://dmpharma.co.in/Ginseng.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 238,
    "name": "METHYLCOBALAMIN  AND ITS COMBINATION",
    "link": "http://dmpharma.co.in/mecofood.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 239,
    "name": "FERROUS ASCORBATE + FOLIC ACID",
    "link": "http://dmpharma.co.in/ferrous.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 240,
    "name": "ACEBROPHYLLINE",
    "link": "http://dmpharma.co.in/acebrophylline.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 241,
    "name": "ESOMEPRAZOLE + DOMPERIDONE",
    "link": "http://dmpharma.co.in/Esomeprazoledomperidone.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 242,
    "name": "GREEN TEA AND ITS COMBINATION",
    "link": "http://dmpharma.co.in/green.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 243,
    "name": "CARBONYL IRON + ZINC SULPHATE  + FOLIC ACID",
    "link": "http://dmpharma.co.in/Carbonyl%20Iron%20.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 244,
    "name": "PREGABALIN  + METHYLCOBALAMIN",
    "link": "http://dmpharma.co.in/Pregablin.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 245,
    "name": "ITRACONAZOLE",
    "link": "http://dmpharma.co.in/Pregablin.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 246,
    "name": "RACECADOTRIL",
    "link": "http://dmpharma.co.in/c1.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 247,
    "name": "THIOCOLCHICOSIDE",
    "link": "http://dmpharma.co.in/Thiocholchicoside.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 248,
    "name": "VITAMIN B COMPLEX  +  MINERALS",
    "link": "http://dmpharma.co.in/Vitamin%20B%20Complex%20with%20Mineral%20Capsules.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 249,
    "name": "CISSUS QUADRANGULARIS EXTRACT",
    "link": "http://dmpharma.co.in/Cissus%20Quadrangularis%20Extract%20Capsule.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 250,
    "name": "GREEN TEA + LYCOPENE + COMBINATION",
    "link": "http://dmpharma.co.in/Green%20Tea,Lycopene%20and%20Combination%20Capsule.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 251,
    "name": "GINSENG + VITAMIN + MINERALS",
    "link": "http://dmpharma.co.in/Ginseng,vitamin%20&%20Mineral%20Capsule.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 252,
    "name": "LYCOPENE + METHYLCOBALAMIN + CAROTENOIDS + MULTIVITAMIN",
    "link": "http://dmpharma.co.in/Lycopene,Methylcobalamin,Carotenoids%20&%20Multivitamin%20Capsule.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 253,
    "name": "PRE - PRO BIOTIC + B-COMPLEX VITAMINS",
    "link": "http://dmpharma.co.in/Pre%20Biotic%20and%20Pro%20Biotic%20with%20B-Complex%20Vitamins%20Capsule.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 254,
    "name": "L-CARNITINE + CO-ENZYME Q10 + ASTAXANTHIN + ZINC SULPHATE MONOHYDRATE + LYCOPENE",
    "link": "http://dmpharma.co.in/L-Carnitine,%20Co-enzyme%20Q10,%20Astaxanthin,%20Zinc%20sulphate%20monohydrate%20and%20Lycopene%20Capsule.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 255,
    "name": "CRANBERRY EXTRACT + D-MANNOSE + HIBISCUS EXTRACT",
    "link": "http://dmpharma.co.in/Cranberry%20Extract,%20D-Mannose%20and%20Hibiscus%20Extract%20Capsule.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 256,
    "name": "LYCOPENE + VITAMINS + MINERAL + GINSENG",
    "link": "http://dmpharma.co.in/Lycopene,%20Vitamins,%20Mineral%20and%20Ginseng%20Capsule.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 257,
    "name": "PANCREATIN",
    "link": "http://dmpharma.co.in/Pancreatin%20Minimicrospheres%20Capsule.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 258,
    "name": "GREEN TEA EXTRACT + LYCOPENE + VITAMINS + MINERALS",
    "link": "http://dmpharma.co.in/Green%20Tea%20Extract,%20Lycopene,%20Vitamins%20and%20Minerals%20Capsule.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 259,
    "name": "NATURAL EXTRACTS + VITAMINS + MINERALS + AMINO ACIDS",
    "link": "http://dmpharma.co.in/Natural%20Extracts,%20Vitamins,%20Minerals%20with%20Amino%20Acids%20Capsules.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 260,
    "name": "RESVERATROL + L\u2013ARGININE",
    "link": "http://dmpharma.co.in/Resveratrol%20and%20L%20%E2%80%93%20Arginine%20Capsules.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 261,
    "name": "CLINDAMYCIN",
    "link": "http://dmpharma.co.in/Clindamycin_Capsule_300mg.html",
    "mainCategory": "Capsules",
    "subCategory": ""
},
{
    "id": 262,
    "name": "PRE-PRO BIOTIC",
    "link": "http://dmpharma.co.in/preprobioticsachet.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 263,
    "name": "CHOLECALCIFEROL",
    "link": "http://dmpharma.co.in/Cholecalciferol.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 264,
    "name": "COLLAGEN PEPTIDES",
    "link": "http://dmpharma.co.in/Collagenpeptides.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 265,
    "name": "RACECADOTRIL",
    "link": "http://dmpharma.co.in/s1.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 266,
    "name": "L-ARGININE",
    "link": "http://dmpharma.co.in/L-Arginine%20sachet.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 267,
    "name": "L-ARGININE + PROANTHOCYANIDIN + VITAMINS + DHA",
    "link": "http://dmpharma.co.in/L-Arginine,%20Proanthocyanidin,%20Vitamins%20with%20Docosahexaenoic%20Acid%20%28DHA%29%20Sachets.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 268,
    "name": "CRANBERRY + D-MANNOSE",
    "link": "http://dmpharma.co.in/Cranberry%20and%20D-Mannose%20Sachet.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 269,
    "name": "COLLAGEN PEPTIDE + GLUCOSAMINE SULPHATE + VITAMIN C",
    "link": "http://dmpharma.co.in/t16.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 270,
    "name": "L-ARGININE + GRAPE SEED EXTRACT",
    "link": "http://dmpharma.co.in/L-Arginine%20and%20Grape%20seed%20extract%20Sachet.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 271,
    "name": "PROBIOTIC + ISABGOL",
    "link": "http://dmpharma.co.in/Probiotic%20With%20Isabgol%20Husk.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 272,
    "name": "GREEN TEA + GARCINIA CAMBOGIA + GINGER EXTRACT + PIPERINE",
    "link": "http://dmpharma.co.in/Green%20Tea,%20Garcinia%20Cambogia,%20Ginger%20Extract%20&%20Piperine%20Sachet.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 273,
    "name": "COLLAGEN + GLUCOSAMINE + BOSWELLIA SERRATA + VITAMINS",
    "link": "http://dmpharma.co.in/Collagen%20+%20Glucosamine%20+%20Boswellia%20Serrata%20+%20Vitamins.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 274,
    "name": "COLLAGEN + ROSEHIP EXTRACT",
    "link": "http://dmpharma.co.in/Collagen%20and%20Rosehip%20Extract%20Sachet.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 275,
    "name": "L-ORNITHINE L-ASPARTATE",
    "link": "http://dmpharma.co.in/L-Ornithine%20L-Aspartate%20Sachet.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 276,
    "name": "L-ARGININE + ZINC + FOLIC ACID",
    "link": "http://dmpharma.co.in/L-Arginine,%20Zinc%20and%20Folic%20Acid%20Sachet.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 277,
    "name": "LYOPHILIZED SACCHAROMYCES BOULARDII",
    "link": "http://dmpharma.co.in/Lyophilized_Saccharomyces_Boulardii_Sachet.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 278,
    "name": "WEIGHT LOSS FORMULA",
    "link": "http://dmpharma.co.in/Alesmin-P%20Weight%20Loss%20Formula%20Sachet.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 279,
    "name": "COLLAGEN + ROSEHIP EXTRACT",
    "link": "http://dmpharma.co.in/Collagen%20and%20Rosehip%20Extract%20Sachet.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 280,
    "name": "SODIUM CHLORIDE",
    "link": "http://dmpharma.co.in/Sodium_%20Chloride_Powder_for_Nasal_Solution.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 281,
    "name": "LACTITOL MONOHYDRATE + ISPAGHULA HUSK",
    "link": "http://dmpharma.co.in/Lactitol%20Monohydrate_Ispaghula%20Husk.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 282,
    "name": "PRE-PRO BIOTIC(STREPTOCOCCUS FAECALIS + CLOSTRIDIUM BUTYRICUM + BACILLUS MESENTERICUS + LACTOBACILLUS SPOROGENES)",
    "link": "http://dmpharma.co.in/Pre-Pro%20Biotic%28Streptococcus%20faecalis,Clostridium%20butyricum,Bacillus%20mesentericus%20and%20Lactobacillus%20sporogenes%29.html",
    "mainCategory": "Sachets",
    "subCategory": ""
},
{
    "id": 283,
    "name": "L-GLUTAMINE + CALCIUM + PHOSPHORUS + L-ARGININE + L-ALANINE + L-LYSINE + MAGNESIUM + VITAMIN C + NIACINAMIDE + VITAMIN E + ZINC + MANGANESE + VITAMIN B6 + COPPER + VITAMIN A + SELENIUM + CHROMIUM + VITAMIN D3",
    "link": "http://dmpharma.co.in/L-Glutamine,%20Calcium,%20Phosphorus,%20L-Arginine,%20L-Alanine,%20L-Lysine,%20Magnesium,%20Vitamin%20C,%20Niacinamide,%20Vitamin%20E,%20Zinc,%20Manganese,%20Vitamin%20B6,%20Copper,%20Vitamin%20A,%20Selenium,%20Chromium%20and%20Vitamin%20D3%20Sachet.html",
    "mainCategory": "Sachets",
    "subCategory": ""
}]

export default items;